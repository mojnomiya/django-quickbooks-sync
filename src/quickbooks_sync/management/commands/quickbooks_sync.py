...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["realm_id"]:
            ...
            self.stdout.write(
                self.style.WARNING("Please provide --realm-id, --all-realms, or --list")
            )
    ...
    def list_realms(self):
        ...
        self.stdout.write(
            f"{'ID':<6} {'Realm ID':<20} {'Company Name':<30} {'Last Sync':<20}"
        )
        self.stdout.write("-" * 76)

        for realm in realms:
            last_sync = (
                realm.last_sync_at.strftime("%Y-%m-%d %H:%M")
                if realm.last_sync_at
                else "Never"
            )
            self.stdout.write(
    ...
    def sync_realm(
        self, realm_id, entity_type=None, direction="from_qbo", async_mode=False
    ):
        ...
        try:
            ...
            if entity_type:
                ...
                if direction == "to_qbo":
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  Synced {len(entities)} {entity_type} entities"
                        )
                    )
            else:
                # Full sync
